const CACHE_NAME = "Thymos-Moira-vLATEST"; // Název cache
const FILES_TO_CACHE = ["./", // Hlavní stránka
    "./index.html", "./uPlot.iife.min.js", "./uPlot.min.css", "./xlsx.full.min.js", "./images/icon-512x512.png", // "./images/icon-192x192.png",
    // "./images/icon-96x96.png",
    // "./images/icon-48x48.png",
    // "./images/icon-32x32.png",
    "./images/favicon.ico",];

const FETCH_EXCEPTIONS = [// Seznam URL, které nebudou cachovány
    "latest.txt",]


// Instalace Service Workeru a cacheování souborů
self.addEventListener('install', function (event) {
    self.skipWaiting().then(_ => {
    });
    try {

        event.waitUntil(caches.open(CACHE_NAME).then((cache) => {
            return Promise.all(FILES_TO_CACHE.map((url) => {
                return fetch(url)
                    .then((response) => {
                        if (!response.ok) {
                            throw new Error(`Failed to fetch ${url}: ${response.statusText}`);
                        }
                        return cache.put(url, response);
                    })
                    .catch((error) => console.error("Caching failed for:", url, error));
            }));
        }));
    } catch (error) {
        console.error("Service Worker installation failed: ", error);
    }
});

// Aktivace Service Workeru
self.addEventListener('activate', function (event) {
    self.clients.claim().then(_ => {
    });

    const cacheWhitelist = [CACHE_NAME];

    event.waitUntil(// Odstraň staré cache + latest.txt ze stávající cache
        caches.keys().then(function (cacheNames) {
            return Promise.all(cacheNames.map(function (cacheName) {
                // ✅ Smaž celé staré cache
                if (!cacheWhitelist.includes(cacheName)) {
                    return caches.delete(cacheName);
                }

                // ✅ Pokud je to aktivní cache, smaž `latest.txt`
                return caches.open(cacheName).then(cache => {
                    return cache.delete('latest.txt');
                });
            }));
        }));
});


// Odezva pro offline přístup
self.addEventListener('fetch', event => {
    const requestUrl = new URL(event.request.url);

    // ✅ Zjisti, jestli žádost spadá do výjimek
    const isException = FETCH_EXCEPTIONS.some(ex => requestUrl.pathname.endsWith(ex));

    if (isException) {
        // 🔁 Normální fetch bez ukládání do cache
        event.respondWith(fetch(event.request));
        return;
    }

    // ⚙️ Standardní cachovací strategie
    event.respondWith(fetch(event.request) // ← 🔍 nejprve se pokusí stáhnout čerstvá data ze sítě
        .then(response => {
            const copy = response.clone();
            caches.open(CACHE_NAME).then(cache => cache.put(event.request, copy)); // uloží novou verzi do cache
            return response; // vrátí síťovou odpověď
        })
        .catch(() => caches.match(event.request)) // pokud síť selže, použije cache
    );
});